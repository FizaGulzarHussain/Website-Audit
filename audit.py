...
def audit_speed(url: str) -> dict:
    ...
    # Varnish / fast.site projection
    ttfb = details.get("ttfb_ms", 1000)
    projected_ttfb = round(ttfb * 0.08)  # Varnish cache hit typically 8–12ms
    details["varnish_projected_ttfb_ms"] = projected_ttfb
    details["varnish_speedup_pct"] = round((1 - projected_ttfb / max(ttfb, 1)) * 100)
...
# ─────────────────────────────────────────────────────────────────────────────
# FAST.SITE VARNISH PROJECTION
# ─────────────────────────────────────────────────────────────────────────────
def compute_fastsite_projection(breakdown: dict) -> dict:
    """
    Generate a marketing-grade fast.site / Varnish caching projection
    ...
    # Varnish cache hit rate on typical CMS/eCommerce: 85-95% of requests
    CACHE_HIT_RATE = 0.90
    # Varnish cache hit serves in ~8-12ms vs origin
    VARNISH_HIT_MS = 10
    # LCP improvement: Varnish + edge delivery typically improves LCP 40-70%
    ...
    projected_ttfb = VARNISH_HIT_MS if ttfb > 100 else ttfb
    ...
    return {
        "algorithm": "Varnish Cache (HTTP accelerator)",
        ...
        "summary_bullets": [
            ...
            f"90% of page requests served instantly from Varnish cache — no origin server wait",
...
def audit_website(url: str, progress_callback=None) -> dict:
    ...
    try:
        ...
        # Could not fetch the page — return a neutral average result with no raw error exposed
        return {
            "url":          url,
            "overall_score": 50,
            "breakdown": {
                "seo":           {"score": 50, "issues": [], "strengths": []},
                "speed":         {"score": 50, "issues": [], "strengths": []},
                "performance":   {"score": 50, "issues": [], "strengths": []},
                "page_ranking":  {"score": 50, "issues": [], "strengths": []},
                "mobile":        {"score": 50, "issues": [], "strengths": []},
                "ddos_security": {"score": 50, "issues": [], "strengths": []},
                "client_reach":  {"score": 50, "issues": [], "strengths": []},
                "trust":         {"score": 50, "issues": [], "strengths": []},
            },
            "lighthouse_details":  {},
            "fastsite_projection": {},
        }
    ...
    # Compute fast.site Varnish projection
